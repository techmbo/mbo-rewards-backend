...
describe("network ops — service assembly", () => {
  it("lists registered networks; seed-only Partnerize is not CONNECTED/HEALTHY", async () => {
        ...
        findMany: mock.fn(async () => [
          {
            ...
            secretRef: "mbo-sm://local-encrypted/network-account/acc-opt",
    ...
    assert.equal(detail.accounts[0].credentialsConfigured, true);
    assert.equal(detail.accounts[0].environment, "PRODUCTION");
    assert.equal(detail.accounts[0].credentialHealth, "HEALTHY");
    assert.ok(Array.isArray(detail.sourceObjects));
    assert.ok(detail.sourceObjects.some((o) => o.sourceObject === "campaigns" && o.live === true));
    assert.equal(JSON.stringify(detail).includes("SECRET_SHOULD_NOT_LEAK"), false);
    assert.equal(JSON.stringify(detail).includes("encryptedAccessToken"), false);
  });
