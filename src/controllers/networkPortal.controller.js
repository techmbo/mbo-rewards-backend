...
export async function mappingRulesHandler(req, res, next) {
  try {
    const { getPagination, toPagedResponse } = await import("../core/pagination.js");
    const { listMappingRegistry } = await import("../modules/mapping/mappingRegistry.service.js");
    const { page, pageSize } = getPagination(req.query);
    const { rows, total } = await listMappingRegistry({
      network: req.query.network || null,
      sourceObject: req.query.source_object || null,
      mappingStatus: req.query.mapping_status || null,
      page,
      pageSize,
      autoSyncIfEmpty: true,
    });
    return res.json({
      ...toPagedResponse({ rows, total, page, pageSize }),
      total,
      items: rows,
      rules: rows,
    });
