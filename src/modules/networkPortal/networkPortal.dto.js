...
import {
  ...
  listSourceOnlyFieldNames,
  PERFORMANCE_RECORD_OBJECT,
...
/**
 * Fill null identity fields from the related SupplierCampaign.
 * Never copies network tracking URL into mboTrackingLink. Never invents metrics or dimensions.
 */
export function hydrateNetworkPerformanceFact(fact) {
  return hydratePerformanceRecordFact(fact);
}
...
export function toNetworkPerformanceDto(row) {
  ...
  return {
    ...
    sourceOnlyFields: meta.sourceOnly && typeof meta.sourceOnly === "object" ? meta.sourceOnly : null,
    sourceOnlyFieldNames: listSourceOnlyFieldNames(meta),
    antiFabricationMetrics: ANTI_FABRICATION_METRICS,
    note: "04A Performance Report (PerformanceRecord). Separate from OrderConversion. networkClicks/linkClicks and mboLinkClicks are independent — never substituted. Missing network metrics stay null.",
  };
}
...
export function toNetworkOrderDto(order) {
  ...
  return {
    ...
    bankReceivedAt: iso(meta.bankReceivedAt ?? null),
