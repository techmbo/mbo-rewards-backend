...
export function extractPerformanceCouponCode(row = {}) {
  ...
  // Without code_id, skip 2-letter tokens (likely country, not coupon).
...
export function mapPerformanceRowToFactInput(row, {
  ...
  const campaignChannelType = deriveCampaignChannelType({
    ...
    trackingUrl: networkTrackingLink,
  });
  ...
  return {
    ...
    impressions: toNum(first(row.impressions, row.Impressions, row.impression)),
    networkClicks,
    ...
    devicePlatform: first(row.devicePlatform, row.device, row.platform, row.Device, null),
    conversionRate: toNum(first(row.conversionRate, row.cr, row.cvr)),
    aov,
    epc: toNum(first(row.epc, row.EPC)),
...
export function aggregatePerformanceRowsByGrain(rows = []) {
  ...
  for (const row of rows || []) {
    ...
    const customerType = normalizeCustomerType(
      first(row.customerType, row.customer_type, row.custType, row.CustType, null),
    );
    const key = [
...
async function enrichFactInputFromCampaignCatalog(input, db = prisma) {
  ...
  if (input.supplierCampaignId && input.brandName && input.networkTrackingLink) return input;
  ...
  return {
    ...input,
    supplierCampaignId: input.supplierCampaignId || campaign.supplierCampaignId || null,
    supplierCampaignDbId: input.supplierCampaignDbId || campaign.id || null,
    brandName: input.brandName || campaign.merchant?.displayName || campaign.merchantNameRaw || null,
    networkTrackingLink: input.networkTrackingLink || campaign.trackingUrl || null,
    campaignSourceId: input.campaignSourceId || source?.id || null,
  };
}
...
/**
 * Fill missing NetworkPerformanceFact.couponCode from Conversion metadata.
 * Optimise /reporting aggregates omit voucher; conversions carry `voucher` → couponCode.
 * Only set when all conversions on that supplier+day share exactly one coupon
 * (never invent / never pick arbitrarily among multiple).
 */
export async function enrichFactsWithConversionCoupons({
  ...
  /** @type {Map<string, string|null>} supplier|yyyy-mm-dd → unique coupon or null if ambiguous/missing */
  const couponByDay = new Map();

  async function uniqueCouponForDay(supplierKey, reportDate) {
    const bounds = dayBoundsUtc(reportDate);
    if (!bounds) return null;
    const dayKey = `${supplierKey}|${reportDate.toISOString().slice(0, 10)}`;
    if (couponByDay.has(dayKey)) return couponByDay.get(dayKey);

    const conversions = await db.conversion.findMany({
      where: {
        supplier: supplierKey,
        conversionDate: { gte: bounds.start, lt: bounds.end },
      },
    ...
    for (const c of conversions) {
      ...
      const hints = meta.attributionHints && typeof meta.attributionHints === "object" ? meta.attributionHints : {};
      const code = first(meta.couponCode, hints.couponCode, meta.voucher, meta.coupon);
      if (code != null && String(code).trim() !== "") codes.add(String(code).trim());
    }
    const unique = codes.size === 1 ? [...codes][0] : null;
    couponByDay.set(dayKey, unique);
    return unique;
  }

  for (const fact of facts) {
    examined += 1;
    const coupon = await uniqueCouponForDay(fact.supplier, fact.reportDate);
    if (!coupon) {
      unresolved += 1;
      continue;
    }

    const channel = String(fact.campaignChannelType || "").toUpperCase();
    const nextChannel =
    ...
    await db.networkPerformanceFact.update({
      ...
      data: {
        couponCode: coupon,
        campaignChannelType: nextChannel || fact.campaignChannelType,
        lastUpdatedAt: new Date(),
      },
    });
    updated += 1;
