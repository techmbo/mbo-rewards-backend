...
export class NetworkPortalService {
  ...
  async rebuildNetworkReconciliation({ billingMonth = null, billingYear = null } = {}) {
    ...
    for (const f of facts) {
      ...
      // Network paid ≠ MBO bank settlement. Never fall back to mboActuallyReceived.
      cur.paidCommission += Number(f.paidCommission ?? 0);
      byKey.set(key, cur);
    }

    // All orders in billing window — drilldown must include pending (not only payable/received).
    ...
    for (const o of orders) {
      ...
      for (const t of o.financialTransactions || []) {
        const v = Number(t.supplierReceivable || 0);
        if (t.transactionType === "REVERSAL") recv -= v;
        else recv += v;
      }
      if (o.supplierPaymentStatus === "PAYMENT_RECEIVED") cur.paidCommission += recv;
      // Confirmed commission stays fact-driven when facts exist; order amounts still fill drilldown.
      if (cur.confirmedCommission === 0 && (o.validationStatus === "VALIDATION_APPROVED" || o.supplierPaymentStatus === "PAYMENT_PAYABLE" || o.supplierPaymentStatus === "PAYMENT_RECEIVED")) {
    ...
    for (const row of byKey.values()) {
      ...
      let status = "UNKNOWN";
      if (row.reportedCommission == null && row.confirmedCommission == null) status = "UNKNOWN";
      else if (Math.abs(confirmedVsPaid) < 0.01 && Math.abs(reportedVsConfirmed) < 0.01) status = "MATCHED";
      else if (row.paidCommission > 0 && confirmedVsPaid > 0.01) status = "PARTIAL";
      else if (row.paidCommission === 0 && row.confirmedCommission > 0) status = "PENDING";
  ...
  async listSupplierCommissionRules({
    ...
    if (totalDb > 0) {
      const items = dbRows.map((row) => {
        const sc = row.campaignSource?.supplierCampaign;
        ...
        const value =
          row.ratePercent != null
            ...
            : row.fixedAmount != null
              ...
              : null;
        const status =
          row.effectiveUntil && new Date(row.effectiveUntil) < new Date()
            ? "EXPIRED"
            : "ACTIVE";
        return {
          id: row.id,
          networkSource: row.supplier,
          brandName: sc?.merchantNameRaw || meta.brandName || null,
          campaignName: sc?.campaignName || meta.campaignName || null,
          supplierCommissionRuleId: meta.supplierRuleId || row.id,
          commissionType: row.supplierRuleType || row.basis || "UNKNOWN",
          commissionValue: value,
          currency: row.currency,
          customerType: meta.customerType || null,
          country:
            Array.isArray(sc?.countryCodes) && sc.countryCodes.length
              ? sc.countryCodes.join(", ")
              : meta.country || null,
          categoryProductGoal: meta.scope || sc?.categoryName || null,
          couponOrTier: meta.couponOrTier || null,
          effectiveFrom: row.effectiveFrom?.toISOString?.() ?? row.effectiveFrom,
          effectiveUntil: row.effectiveUntil?.toISOString?.() ?? row.effectiveUntil,
          ruleStatus: status,
          sourceFieldPath: meta.sourceFieldPath || "SupplierCommissionRule",
          mappingStatus: meta.mappingStatus || "MAPPED",
          lastModifiedAt: row.updatedAt?.toISOString?.() ?? row.updatedAt,
        };
      });
    ...
    const items = camps.map((sc) => {
      const unit = String(sc.commissionUnit || "").toUpperCase();
      const rawVal = sc.defaultCommissionValue != null ? Number(sc.defaultCommissionValue) : null;
      const commissionValue =
        rawVal == null || Number.isNaN(rawVal)
          ? null
          : unit === "PERCENT"
            ? `${rawVal}%`
            : String(rawVal);
      const groups = Array.isArray(sc.commissionGroups) ? sc.commissionGroups : [];
      return {
        id: `projected:${sc.id}`,
        networkSource: sc.supplier,
        brandName: sc.merchantNameRaw,
        campaignName: sc.campaignName,
        supplierCommissionRuleId: `SCR-${sc.supplier}-${sc.supplierCampaignId || sc.id.slice(0, 8)}`,
        commissionType: unit === "PERCENT" ? "PERCENT" : unit === "FLAT" ? "FIXED" : "OTHER",
        commissionValue,
        ...
        effectiveFrom: null,
        effectiveUntil: null,
        ruleStatus: groups.length || rawVal != null ? "ACTIVE" : "NEEDS_REVIEW",
        sourceFieldPath:
          unit === "PERCENT"
            ? "default_commission_rate / commission"
            : "defaultCommissionValue / commissionGroups",
        mappingStatus: groups.length || rawVal != null ? "MAPPED" : "NEEDS_REVIEW",
        lastModifiedAt: sc.updatedAt?.toISOString?.() ?? sc.updatedAt,
        projected: true,
      };
    });
