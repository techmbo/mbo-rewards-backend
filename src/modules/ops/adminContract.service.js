...
} from "../reporting/attributionLogic.contract.js";
import {
...
export class AdminContractService {
  ...
  async listPerformance(
    ...
    if (grainNorm === "network" || grainNorm === "network_fact") {
      ...
      return {
        ...
        unavailableFields: ANTI_FABRICATION_METRICS,
        grainNote:
  ...
  async listOrders(
    ...
    const items = rows.map((order) => {
      ...
      // Client: order FK or ATTRIBUTED conversion assignment only — never guess from coupon catalog.
      const displayClient = resolveDisplayClientFromConversion(conv, order);
      let clientName = displayClient.clientName ?? order.client?.name ?? null;
      let clientId = displayClient.clientId ?? order.clientId ?? null;
      ...
      const clientPayableEligible = resolveClientPayableEligibility({
        order,
        financialTransactions: order.financialTransactions || [],
      }).eligible;
      ...
      return toAdminOrderDto(
        {
          ...
          networkContext: {
            ...
            networkConversionId:
              conv?.supplierConversionId ??
              meta.networkConversionId ??
              convMeta.networkConversionId ??
              null,
            dedupeKey:
              meta.dedupeKey ??
              convMeta.dedupeKey ??
              (conv
                ? `${order.supplier}|${order.sourceAccountLabel || "default"}|${conv.supplierConversionId}`
                : null),
            mboCanonicalObject: "OrderConversion",
            attributionStatus: conv?.attributionStatus ?? null,
            attributionEvidence:
              convMeta.attributionEvidence ??
              convMeta.attributionReview?.evidence ??
              null,
            attributionEvidenceLabel: attributionEvidenceLabel(
              convMeta.attributionEvidence ?? convMeta.attributionReview?.evidence ?? null,
            ),
            attributionReviewReason:
              convMeta.attributionReview?.reason ??
              (conv?.attributionStatus === "REVIEW_REQUIRED" ? "review_required" : null),
            attributionReviewReasonLabel: attributionReviewReasonLabel(
              convMeta.attributionReview?.reason ?? null,
            ),
            clientAssignmentId: conv?.clientAssignmentId ?? order.clientAssignmentId ?? null,
            ...
            rawStatus:
              meta.network_raw_status ??
              meta.networkRawStatus ??
              meta.rawStatus ??
              meta.supplierStatus ??
              convMeta.network_raw_status ??
              convMeta.rawStatus ??
              conv?.status ??
              null,
            networkRawStatus:
              ...
              meta.networkRawStatus ??
              convMeta.network_raw_status ??
              convMeta.networkRawStatus ??
              meta.rawStatus ??
              convMeta.rawStatus ??
              null,
            mboOrderStatus:
              meta.mboOrderStatus ??
              convMeta.mboOrderStatus ??
              null,
            confirmedDate,
            paymentConfirmedDate: meta.paymentConfirmedDate ?? convMeta.paymentConfirmedDate ?? null,
            mboReceivedDateTime: formatMboReceivedDateTime(mboReceipt),
            bankReceivedAt: mboReceipt?.dateTime ?? null,
            mboReceivedAmount: mboReceipt?.amount ?? null,
            clientPayableEligible,
            networkPaymentEvidence: meta.networkPaymentEvidence ?? meta.networkPaymentStatus ?? null,
            paymentStatus,
