 ...
 *
 * Ingest order is enforced by src/modules/networkOps/pipeline:
 * fetch → immutable raw payload → schema → versioned mapping → MBO canonical →
 * validate → upsert → attribution → commercial → network ops → finance →
 * client-safe MBO model. Never skip raw. Never map a network response into the client API.
 */
