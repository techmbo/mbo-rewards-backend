...
import { runOrderReconciliationChecks } from "../finance/reconciliation.service.js";
...
export class AdminNetworkFinanceService {
  ...
  async listNetworkPaymentsReceived({
    ...
    let expanded = rows.map(({ order, billingMonth: bm, billingYear: by, payable, paymentUi, anchor, brandName }) => {
      ...
      const reconResult = runOrderReconciliationChecks({
        order,
        financialTransactions: order.financialTransactions,
      });
      const reconUi = reconResult.allMatched
        ? "FULLY_MATCHED"
        : reconResult.hasMaterialMismatch
          ? "MISMATCH"
          : mboReceipt
            ? "PARTIALLY_MATCHED"
            : paymentUi === "PAID" || paymentUi === "INVOICED"
              ? "PARTIALLY_MATCHED"
      ...
      return {
        ...
        mboReceivedDate: mboReceipt?.date ?? null,
        mboReceivedTime: mboReceipt?.time ?? null,
        ...
        billingMonth: bm,
        billingYear: by,
        matchedInvoiceAmount: mboReceipt ? payable : 0,
        ...
        mboReceiptStatus: mboReceipt ? "RECEIVED" : "NOT_RECONCILED",
        clientPayableEligible: resolveClientPayableEligibility({
          order,
          financialTransactions: order.financialTransactions,
    ...
    const mboAmountReceived = expanded
      .filter((x) => x.mboReceivedDateTime)
  ...
  /**
   * v13 MBO Receipts — internal bank/reconciliation facts only (never network PAID).
   */
  async listMboReceipts({
    ...
    const receipts = (payments.items || [])
      .filter((p) => p.mboReceivedDateTime)
      .map((p, idx) => ({
        id: `rcpt:${p.id}`,
        ...
        mboReceivedDateTime: p.mboReceivedDateTime,
        mboReceivedDate: p.mboReceivedDate,
        mboReceivedTime: p.mboReceivedTime,
        amountReceived: p.paymentAmount ?? null,
        currency: p.currency || p.paymentCurrency || null,
        evidenceSource: p.mboReceiptStatus === "RECEIVED" ? "Bank / reconciliation fact" : "—",
        reconciliationStatus:
          p.reconciliationStatus === "FULLY_MATCHED"
            ...
            : p.reconciliationStatus === "PARTIALLY_MATCHED"
              ...
              : "NOT_RECONCILED",
        recordedBy: "System",
        clientPayableEligible: p.clientPayableEligible ?? false,
      }));
