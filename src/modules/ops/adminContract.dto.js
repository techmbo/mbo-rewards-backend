...
export function toAdminOrderDto(order, { includeFinancial = false } = {}) {
  ...
  let mboStatus = nc.mboStatus ?? null;
  if (!mboStatus) {
  ...
  const base = {
    ...
    rawStatus: nc.rawStatus ?? null,
    mboStatus,
    orderStatus: mboStatus,
    ...
    bankReceivedAt: iso(nc.bankReceivedAt || order.supplierPaymentChangedAt),
  ...
  const received =
    String(payment).toUpperCase() === "PAYMENT_RECEIVED" ? payable : money(nc.receivedCommission ?? null);
  return {
    ...base,
