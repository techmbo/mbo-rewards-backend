...
import { buildConversionDedupeKey } from "../../order/orderConversionIngestion.contract.js";
...
} from "../attributionLogic.contract.js";
...
export class AttributionService {
  ...
  async ingestConversion(input, client = null) {
    ...
    const baseData = {
      ...
      metadata: {
        ...
        networkConversionId: input.supplierConversionId,
  ...
  async attributeConversion(conversionId, client = null, options = {}) {
      ...
      if (assignment?.clientId && String(assignment.clientId) !== String(clientIdHint)) {
        try {
          await this.exceptions.report(
            {
              condition: ALERT_CONDITION.ATTRIBUTION_UNRESOLVED_ORPHAN,
    ...
    if (context.reviewRequired) {
      ...
      return this.conversionRepo.update(
        conversionId,
        {
          ...
          metadata: {
            ...asObject(conversion.metadata),
            attributionReview: buildAttributionReviewMetadata({
              reason: context.reviewReason || ATTRIBUTION_REVIEW_REASON.AMBIGUOUS,
    ...
    const metadata = buildAttributedMetadata(conversion.metadata, {
      evidence: context.evidence ?? null,
    });
    if (commissionUnresolvedReason) {
      metadata.commissionUnresolvedReason = commissionUnresolvedReason;
    } else {
      metadata.commissionUnresolvedReason = null;
    }
  ...
  async resolveAttributionContext(conversion, client = null, options = {}) {
    ...
    if (click) evidence = ATTRIBUTION_EVIDENCE.MBO_CLICK;
    else if ((conversion.subId || hints.subId) && clientAssignmentId) {
      evidence = ATTRIBUTION_EVIDENCE.NETWORK_SUBID;
    } else if (trackingLink) evidence = ATTRIBUTION_EVIDENCE.TRACKING_LINK;
    ...
    if (!clientAssignmentId && assignmentIdHint) {
      ...
      if (assignment) {
        ...
        evidence = evidence || ATTRIBUTION_EVIDENCE.ASSIGNMENT_HINT;
        if (!trackingLink) {
    ...
    if (!clientAssignmentId && couponCode && this.couponAssignmentRepo?.findActiveByCouponCode) {
      ...
      if (assignmentIds.length === 1) {
        ...
        reviewRequired = true;
        reviewReason = ATTRIBUTION_REVIEW_REASON.SHARED_COUPON;
      }
    }
    ...
    if (!clientAssignmentId && !reviewRequired && campaignSourceId) {
      ...
      if (singles.length === 1) {
        ...
        evidence = ATTRIBUTION_EVIDENCE.SINGLE_ASSIGNMENT;
        if (!trackingLink) {
          trackingLink = await this.trackingRepo.findPrimaryForAssignment(clientAssignmentId, client);
        }
      } else if (singles.length > 1) {
        reviewRequired = true;
        reviewReason = ATTRIBUTION_REVIEW_REASON.MULTIPLE_ASSIGNMENTS;
    ...
    return {
      ...
      couponCode,
      attributionPriority: classifyAttributionPriority({ evidence, reviewRequired }),
    };
  }
}
