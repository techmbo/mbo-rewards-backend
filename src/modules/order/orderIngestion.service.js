...
} from "./orderConversionIngestion.contract.js";
import {
  buildOrderPatch,
  isPresent,
...
import {
  buildNetworkPaymentEvidenceMetadata,
...
export class OrderIngestionService {
  ...
  async upsertOrder(input, client = null) {
    ...
    const effectiveMboOrderStatus =
      input.mboOrderStatus ??
      statusResolution.mboOrderStatus ??
      (input.legacyConversionStatus
        ? resolveOrderStatusFromNetworkRaw(input.legacyConversionStatus, { supplier }).mboOrderStatus
        : null);

    const validationStatus =
      input.validationStatus ||
      (effectiveMboOrderStatus
        ? mapMboOrderStatusToValidation(effectiveMboOrderStatus)
        : statusResolution.mappingExceptionRequired || input.statusMappingExceptionRequired
          ? "VALIDATION_NEEDS_REVIEW"
          : input.legacyConversionStatus
            ? mapConversionStatusToValidation(input.legacyConversionStatus)
            : null);

    const supplierPaymentStatus = input.supplierPaymentStatus ?? null;

    const rawNetworkPayment =
      input.networkRawStatus ??
      statusResolution.networkRawStatus ??
      input.metadata?.network_raw_status ??
      null;
    if (rawNetworkPayment && isNetworkPaymentEvidenceOnly(rawNetworkPayment)) {
      try {
        await this.exceptions.report(
          {
            type: "MAPPING_UNMAPPED_CRITICAL_FIELD",
            severity: "LOW",
            supplier,
            reason: "network_payment_evidence_recorded",
            metadata: {
              networkPaymentEvidence: String(rawNetworkPayment),
              note: "Network payment status stored as evidence only — not MBO bank receipt",
            },
          },
          db,
        );
    ...
    if (statusResolution.mappingExceptionRequired || input.statusMappingExceptionRequired) {
      try {
        await this.exceptions.report(
          {
            condition: ALERT_CONDITION.UNKNOWN_SOURCE_STATUS,
            type: "MAPPING_UNKNOWN_ENUM",
            supplier,
            reason: "unknown_network_order_status",
            metadata: {
              networkRawStatus: statusResolution.networkRawStatus,
              supplierOrderId,
              sourceAccountLabel,
            },
          },
          ...
          mergeMetadata(
            {
              ...
              networkConversionId: input.supplierConversionId ?? null,
            },
            input.metadata,
          ),
          rawNetworkPayment && isNetworkPaymentEvidenceOnly(rawNetworkPayment) ? rawNetworkPayment : null,
        ),
        {
          networkRawStatus:
            input.networkRawStatus ??
            ...
            input.metadata?.network_raw_status ??
            null,
          mboOrderStatus: effectiveMboOrderStatus,
          mappingExceptionRequired:
            statusResolution.mappingExceptionRequired || input.statusMappingExceptionRequired || false,
        },
      ),
    };
  ...
  async upsertItems(orderId, items, client = null) {
    ...
    for (let i = 0; i < items.length; i += 1) {
      const item = items[i] || {};
      const lineKey = resolveOrderItemLineKey(item, i);
