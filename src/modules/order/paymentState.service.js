...
import { resolveClientPayableEligibility } from "../finance/financeSeparation.contract.js";
...
export class PaymentStateService {
  ...
  async transitionClientPayment(orderId, toStatus, { reason, actorId } = {}, client = null) {
    ...
    if (toStatus === "CLIENT_PAYMENT_PAYABLE" && order.validationStatus === "VALIDATION_APPROVED") {
      ...
      const recon = runOrderReconciliationChecks({
        order,
        financialTransactions: fts || [],
      });
      const eligibility = resolveClientPayableEligibility({
        order,
        financialTransactions: fts || [],
        reconciliationChecks: recon.checks,
      });
      if (!eligibility.eligible) {
        throw fail(eligibility.reason || "Client payment cannot become PAYABLE yet.", 409);
      }
    }
