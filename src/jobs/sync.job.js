 ...
 */
async function syncAccountsWithConcurrency(accountLabels, syncFn) {
  const result = {};
  await runWithConcurrency(accountLabels, SYNC_ACCOUNT_CONCURRENCY, async (label) => {
    try {
      ...
      result[label] = { failed: true, error: error?.message || "Sync failed" };
...
async function syncBoostinyAccount(accountLabel) {
  ...
  const payload = await adapter.fetchAll(
    {
      performance: { from, to },
      linkPerformance: { from, to },
    },
    {
      skipCampaigns: !refreshCampaigns,
      skipCoupons: !refreshCoupons,
    },
  );
  accountTimer.end("apiFetchMs");

  const campaignsForEnrichment = refreshCampaigns
    ? payload.campaigns
  ...
  const factPromo = await promotePerformanceRowsToFacts(detailPerformanceRows, {
    networkSource: "boostiny",
  ...
  await markAccountSyncSuccess("boostiny", accountLabel, {
    ...
    refreshedOrders: boostinyConversionRows.length > 0 || detailPerformanceRows.length > 0,
    refreshedPayments: boostinyPayments.length > 0,
  });
  ...
  return {
    ...
    payments: boostinyPayments.length,
...
async function syncBoostiny() {
  ...
  return syncAccountsWithConcurrency(accountLabels, syncBoostinyAccount);
}
...
async function syncOptimiseRegion(region, accountLabel) {
  ...
  // Phase 4 — independent Optimise resources fetched concurrently; rate limiter serializes calls.
  const [
    campaignsResult,
    conversionsResult,
    ...
    refreshCampaigns
      ? fetchOptimiseResource("campaigns", credentials, () => adapter.fetchCampaigns())
      : fetchOptimiseResource("campaigns", credentials, async () => [], {
          skipped: true,
          skipReason: "cache",
        }),
    fetchOptimiseResource("conversions", credentials, () => adapter.fetchConversions(conversionsRange)),
    fetchOptimiseResource("conversionsByPayment", credentials, () =>
      adapter.fetchConversions({
        ...conversionsRange,
        conversionType: "conversionsByPayment",
      }),
    ),
    fetchOptimiseResource("reporting", credentials, () => adapter.fetchReporting(reportingRange)),
    fetchOptimiseResource("invoiceReporting", credentials, () =>
      adapter.fetchInvoiceReporting(reportingRange),
    ),
    fetchOptimiseResource("payments", credentials, () => adapter.fetchPayments(paymentsRange)),
    fetchOptimiseResource("invoices", credentials, () => adapter.fetchInvoices(paymentsRange)),
    refreshCoupons
      ? fetchOptimiseResource("voucherCodes", credentials, () => adapter.fetchVoucherCodes())
      : fetchOptimiseResource("voucherCodes", credentials, async () => [], {
          skipped: true,
          skipReason: "cache",
        }),
  ]);
  ...
  await upsertManyRawEntities({
    networkSource,
    entityType: "performance",
    rows: [...reportingRows, ...invoiceReportingRows],
  ...
  // Coupon enrich runs again after conversion-promotion (maybePromoteAfterSync) once Conversion rows exist.
...
async function syncTrackierAccount(accountLabel) {
  ...
  const profileResult = await fetchTrackierResource("profile", credentials, () => adapter.fetchProfile());

  const pubId = profileResult.data?.id ?? profileResult.data?.hashId ?? null;

  const reportsKpiResult = await fetchTrackierResource("reportsKpi", credentials, () =>
  ...
  const [
    ...
    refreshCampaigns
      ? fetchTrackierResource("categories", credentials, () => adapter.fetchCategories())
      : fetchTrackierResource("categories", credentials, async () => [], {
          skipped: true,
          skipReason: "cache",
        }),
    pubId
      ? fetchTrackierResource("campaignsCount", credentials, () => adapter.fetchCampaignsCount(pubId))
      : fetchTrackierResource("campaignsCount", credentials, async () => null, {
          skipped: true,
          skipReason: "missing_publisher_id",
        }),
    refreshCampaigns
      ? fetchTrackierResource("campaigns", credentials, () => adapter.fetchCampaigns())
      : fetchTrackierResource("campaigns", credentials, async () => [], {
          skipped: true,
          skipReason: "cache",
        }),
    refreshCoupons
      ? fetchTrackierResource("coupons", credentials, () => adapter.fetchCoupons())
      : fetchTrackierResource("coupons", credentials, async () => [], {
          skipped: true,
          skipReason: "cache",
        }),
    refreshCoupons
      ? fetchTrackierResource("deals", credentials, () => adapter.fetchDeals())
      : fetchTrackierResource("deals", credentials, async () => [], {
          skipped: true,
          skipReason: "cache",
        }),
    fetchTrackierResource("conversions", credentials, () =>
      adapter.fetchConversions({
        startDate: dateRange.start,
        endDate: dateRange.end,
      }),
    ),
    fetchTrackierResource("reports", credentials, () =>
  ...
  const factPromo = await promotePerformanceRowsToFacts(reportingRows, {
    networkSource: "trackier",
    sourceAccountLabel: accountLabel || "default",
...
async function syncTrackier() {
  ...
  return syncAccountsWithConcurrency(accountLabels, syncTrackierAccount);
}
...
async function maybePromoteAfterSync(result) {
  ...
  // Optimise reporting omits voucher — fill NetworkPerformanceFact.couponCode from Conversion.
  try {
    const couponEnrich = await enrichFactsWithConversionCoupons({});
    ...
    if (platform === "boostiny") {
      ...
      if (accountLabel) {
        ...
        if (accountLabels.length === 0) {
          ...
          result = {
            [region]: await syncAccountsWithConcurrency(accountLabels, (label) =>
              syncOptimiseRegion(region, label),
            ),
          };
        }
      }
